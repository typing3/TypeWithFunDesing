...
function toggleSettingsPanelOverlay() {
  settingsDrawer.classList.toggle('drawer-open-focus');
}

function applyGlobalColorTheme(themeName) {
  document.body.setAttribute('data-theme-palette', themeName);
}

function toggleLightDarkModeSystem() {
  const isChecked = document.getElementById('lightDarkSwitch').checked;
  if (isChecked) {
    document.body.setAttribute('data-mode-switch', 'light');
  } else {
    document.body.removeAttribute('data-mode-switch');
  }
}
